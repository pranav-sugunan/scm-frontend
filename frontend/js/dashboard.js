...
async function loadDashboard(silent = false) {
  ...
  try {
    ...
    const lowStock = belowReorder.status === 'fulfilled' ? belowReorder.value : [];
    ...
    if (dash) {
      ...
      showError('kpi-container', 'Could not load dashboard. Ensure the backend is running on port 8000.');
    }
  } catch (err) {
...
function renderKPIs(dash, lowStock = []) {
  ...
  const pendingOrders    = Math.round((orderKpis.backorder_rate ?? 0) * totalOrders) || (dash.pending_orders ?? 0);
  ...
  const kpis = [
    {
      ...
      icon: orderIcon(),
      color: 'blue',
      trend: pendingOrders > 0 ? { dir: 'up', label: `${pendingOrders} active` } : null,
    },
    {
      id: 'inventory-levels',
      ...
      trend: lowStockCount > 0 ? { dir: 'down', label: `${lowStockCount} alerts` } : { dir: 'up', label: 'Healthy' },
    },
    {
      ...
      trend: delayedShipments > 0 ? { dir: 'down', label: `${delayRate}% rate` } : { dir: 'up', label: 'On time' },
    },
    {
      id: 'low-stock-alerts',
      ...
      trend: { dir: lowStockCount > 0 ? 'down' : 'up', label: `${stockHealth}% healthy` },
    },
  ];
...
function renderOrdersChart(dash) {
  ...
  const labels = ['Fulfillment', 'On-Time Delivery', 'Perfect Order', 'Backorder'];
  const values = [
  ...
  ordersChart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Rate (%)',
        data: values,
        backgroundColor: [CHART_COLORS.primary, CHART_COLORS.success, CHART_COLORS.info, CHART_COLORS.warning],
        borderWidth: 0,
        borderRadius: 6,
        barPercentage: 0.6,
      }],
...
function renderInventoryChart(dash) {
  ...
  const distribution = dash.inventory_distribution
    ?? dash.inventory_by_category
    ?? [];
  ...
  if (distribution.length > 0) {
    ...
    const healthy      = Math.max(0, (invKpis.total_skus ?? 0) - belowReorder - overstock);
...
async function renderRecentOrders() {
  ...
  try {
    ...
    orders = Array.isArray(data) ? data.slice(0, 8) : (data?.items ?? []).slice(0, 8);
  } catch (_) { /* silently skip */ }

  if (orders.length === 0) {
    showEmpty('recent-orders-container', 'No recent orders', 'Orders will appear here once created.');
    return;
  }
...
function renderAlerts(lowStock = []) {
  ...
  container.innerHTML = `
    ...
    ${lowStock.length > 5 ? `<div class="text-sm text-secondary" style="margin-top:.5rem;padding:.5rem">
